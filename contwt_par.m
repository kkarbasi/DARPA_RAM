...
%
%  if Y is a matrix, time is along dimension 1 (column)
%
...
x = Y - repmat( mean(Y,1), size(Y, 1), 1 );

disp('inside contwt - 3')

if (pad == 1)
        if numel(y) ~= length(y)
              error('we didn''t adjust this code for zero-padding')
        end
	base2 = fix(log(n1)/log(2) + 0.4999);   % power of 2 nearest to N
	x = [x,zeros(1,2^(base2+1)-n1)];
end
disp('inside contwt - 4')
...
disp('inside contwt - 5')

%....compute FFT of the (padded) time series
f = fft(x);    % [Eqn(3)]
disp('inside contwt - 6 - after fft')

%....construct SCALE array & empty PERIOD & WAVE arrays
scale = s0*2.^((0:J1)*dj);
period = scale;
%wave = zeros(J1+1,n);  % define the wavelet array
wave = zeros(size(x, 2), J1+1, n);  % define the wavelet array
wave = wave + i*wave;  % make it complex
disp('inside contwt - 7')
